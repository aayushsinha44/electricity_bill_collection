...
$user_id = ""; $name = "a"; $address = ""; $phone_number = ""; $email = "";$bill = ""; $collector_id = "";
if(isset($_GET["id"])){
  $id = $_GET["id"];
  $q1 = "SELECT * FROM consumer WHERE user_id='$id'";
  ...
  $email = $row1["email"];
  $bill = $row1["total_amount_to_be_paid"];
  $collector_id = $row1["collector_id"];
}
else{
  die("404 Not found");
}
...
if(isset($_POST["btn"])){
    ...
    $bill = $_POST["bill"];
    echo $name.' '.$address.' '.$phone_number.' '.$email.' '.$collector_id.' '.$bill;
    $q = "UPDATE consumer SET name = '$name', address='$address', phone_number='$phone_number', email='$email', collector_id='$collector_id', total_amount_to_be_paid='$bill'  WHERE user_id = '$user_id'";
    $r = mysqli_query($conn,$q);
    if($r){
         ?><script type="text/javascript">alert("Updated Successfully");</script><?php
    header("Location: consumer.php?update=1");
    }
    else{
        ...
        <main class="col-sm-9 ml-sm-auto col-md-10 pt-3" role="main">
          ...
          <form class="col-lg-3" method="POST">
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control" name="collector_id" value="<?php  echo $collector_id; ?>">
            </div>
            <div class="form-group">
              <label for="pwd">Bill:</label>
              <input type="number" class="form-control" name="bill" value="<?php  echo $bill; ?>">
            </div>
