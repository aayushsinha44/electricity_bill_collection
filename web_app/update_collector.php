...
$user_id = ""; $name = "a"; $address = ""; $phone_number = ""; $email = "";$collected = ""; $transfered = ""; $supervisor_id = "";
if(isset($_GET["id"])){
  $id = $_GET["id"];
  $q1 = "SELECT * FROM collector WHERE user_id='$id'";
...
if(isset($_POST["btn"])){
    ...
    $q = "UPDATE collector SET name = '$name', address='$address', phone_number='$phone_number', email='$email', supervisor_id='$supervisor_id'  WHERE user_id = '$user_id'";
    $r = mysqli_query($conn,$q);
    if($r){
         ?><script type="text/javascript">alert("Updated Successfully");</script><?php
    header("Location: collector.php?update=1");
    }
    else{
